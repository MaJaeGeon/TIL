...
# 데이터베이스의 개념적 구조
1975년 ANSI(American National Standards Institute)에서 데이터베이스를 개념적으로 이해하기 위해  
데이터베이스 구조에 관한 안을 만들었는데 이 안은 데이터베이스를 보는 관점을 세 단계를 분리했기 때문에  
3단계 데이터베이스 구조라고 부른다.

3단계 데이터베이스 구조는 외부 단계, 개념 단계, 내부 단계로 나뉘어지는데 각 단계를 설명하는데  
사용되는 용어인 `스키마(scheme)`는 그리스어에서 유래된 단어로 조직이나 구조를 의미한다.

- 외부 단계
  일반 사용자나 프로그래머가 접근하는 계층으로 데이터베이스 중에서  
  하나의 논리적인 부분을 의미하며, 여러개의 외부 스키마가 있을 수 있다.
  외부 스키마는 서브 스키마라고도 부르며, 뷰의 개념이다. 

- 개념 단계
  전체 데이터베이스의 정의를 의미하는것으로 하나의 데이터베이스에는 하나의 개념 스키마가 있으며, DBA가 관리한다.
  개념 스키마는 저장장치에 독립적으로 기술되며, 데이터와 관계, 제약사항, 무결성에 대한 내용이 포함된다.

- 내부 단계
  물리적 장치에 데이터베이스가 실제로 저장되는 방법을 표현한다.  
  내부스키마는 하나이며, 인덱스, 데이터 레코드의 배치 방법, 데이터 압축 등에 관한 사항이 포함된다.  

DBMS는 매핑을 통해 각 단계 간 대응 관계를 정의하는데 매핑은 개념 스키마의 어느 부분이 외부 스키마가 되는지 또는  
개념 스키마가 어떤 물리적인 표현을 통해 내부 스키마가 되는지에 따라 두가지로 나뉜다.

- 외부/개념 매핑 : 사용자의 외부 스키마와 개념 스키마 간의 매핑으로  
...
데이터 독립성은 하위 단계의 내용을 추상화하여 상위 단계에 그 세부 사항을 숨김으로써 한 단계 내의  
변경에 대해서 다른 단계와 상호 간섭이 없도록 하는 것이다.  
...
- 물리적 데이터 독립성(Physical Data Independence)
  개념 단계와 내부 단계 사이의 독립성으로, 저장장치 구조 변경과 같이 내부 스키마가 변경되어도  
  개념스키마에 영향을 미치지 않도록 지원한다.